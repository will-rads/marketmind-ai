...
**2026-05-14 night — page-break hygiene hardening:** Will sent a screenshot showing a section's last sentence dangling onto a new page and asked to prevent stranded lines and image breakages "so this does not happen again." Root cause: the preamble had section/figure `\Needspace` guards and `placeins[section]` but no widow/orphan penalties. Added to the preamble: `\widowpenalty`, `\clubpenalty`, `\displaywidowpenalty`, `\brokenpenalty` all at 10000; `\raggedbottom` so the penalties resolve without "Underfull vbox" warnings; and `\pretocmd{\subsubsection}{\Needspace{6\baselineskip}}` so subsubsection headings get the same anti-orphan guard `\section`/`\subsection` already had. The `fig:metablock` wrapfigure was left untouched (it wraps correctly as-is; altering a working wrapfig risks regressing it). Preamble-only change. **Commit status:** the clarification patch (commit `f18d908`) was pushed to `origin/codex/latex-round6-compile`; the page-break hardening commit was prepared but the push was paused by Will before it landed.

**2026-05-14 night — Hammoud's tone note on the Section 1.4 opener:** Hammoud flagged the opening paragraph of Section 1.4 (Contributions of the thesis). His objection was tone, not content: the phrases "the contribution at the level the project should be judged" and "none of them, taken in isolation, is the contribution" read as defensive and as instructing the examiner how to grade. He asked for the same idea rewritten as a plain description of what MarketMind is — a single platform that unifies the workflow — using positive framing along the lines of "MarketMind comprises several pieces…" and "streamlines the whole workflow in one platform … rather than relying on fragmented solutions." Claude drafted a replacement paragraph that keeps every fact from the original (the six component pieces, the cross-reference to Chapter 3), drops both flagged phrases, and leads with the one-platform-vs-fragmented-tools framing. Will reviewed it, sent it to Hammoud in chat, and **Hammoud approved it (2026-05-14 night).** Queued for the batch implementation below; when applied, the literal "Chapter 3" is restored to `\Cref{ch:approach}` so the cross-reference stays renumber-safe.

**2026-05-14 night — Hammoud's figure-reference consistency note:** Hammoud flagged that figure references are not consistent (his example: "Figure 3.5 ... fig. 3.1" from the line-506 sentence). Root cause: the report uses `cleveref` with no `\crefname` override, so `\Cref{fig:...}` renders "Figure" (15 call sites) and `\cref{fig:...}` renders "figure" (21 call sites) — a real capitalization split, not an abbreviation issue (cleveref never abbreviates to "fig." by default). Decision: standardize on **"Figure"** (capitalized, full word) everywhere, the dominant academic convention. Fix is a two-line preamble addition — `\crefname{figure}{Figure}{Figures}` + `\Crefname{figure}{Figure}{Figures}` — which forces both `\cref` and `\Cref` to render "Figure" regardless of call site or sentence position; no per-call-site edits needed. Same treatment to be applied to tables. Sections/chapters have the same split but were not flagged; left as-is for now.

**2026-05-14 night — Hammoud's single-column note (pages 29–30):** Hammoud flagged that pages 29–30 "interchangeably switch to 2-column / 1-column writing style" and asked to "stick with 1 column only." Root cause: the `fig:metablock` `wrapfigure` in subsubsection 3.5.1.5 (Meta Ads implementation mapping) wraps body text down its side, producing a two-column look. Plan: convert the `wrapfigure` to a normal centered `\begin{figure}` (single-column flow, text above and below, not beside) at a moderate width since the image is portrait; drop the "on the right" phrasing from the prose; remove the now-unused `\usepackage{wrapfig}`. Will's concern: the conversion pushes text down and could disturb page 31, where the `fig:metablock-meta` 2x2 grid of Meta Ads Manager screenshots sits. Mitigation: the 2x2 grid is a single `figure` with four `subfigure`s, so it is atomic and cannot be split across pages — worst case it floats to its own page, which is the desired "all shown together" outcome anyway. Exact pagination can only be confirmed by the GitHub Actions compile; if page 31 lands badly, the `fig:metablock` width is the single knob to nudge.

**Consolidated Round 6 night batch — implemented in one go (2026-05-14 night, Will's go-ahead given):**

1. **§1.4 opener rewrite** — done. The line-259 paragraph now reads as a plain description ("MarketMind streamlines the whole campaign workflow inside one platform…"); `\Cref{ch:approach}` preserved.
2. **Page-break hygiene** — done. `\widowpenalty`/`\clubpenalty`/`\displaywidowpenalty`/`\brokenpenalty` at 10000, `\raggedbottom`, `\pretocmd{\subsubsection}{\Needspace{6\baselineskip}}`.
3. **Figure-reference consistency** — done. `\crefname`/`\Crefname` added for `figure` and `table`; all references now render the capitalised full word.
4. **Single-column fix** — done. `fig:metablock` `wrapfigure` converted to a centered `\begin{figure}[htbp]` at `width=0.5\textwidth`; "on the right" phrasing dropped; `\usepackage{wrapfig}` removed.

**Verification:** `\begin`/`\end` balanced (93/93); zero duplicate labels (122 total); all 74 `\cref`/`\Cref` targets resolve; zero `wrapfig` references remaining. Committed and pushed to `origin/codex/latex-round6-compile` to trigger the GitHub Actions LaTeX compile. **Still to confirm visually:** pages 29–31 in the compiled PDF — that the de-wrapped `fig:metablock` reflows cleanly single-column and the `fig:metablock-meta` 2x2 grid stays intact (if page 31 lands badly, the `fig:metablock` width is the knob to nudge).
