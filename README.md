...
**Status:** Direction locked — architecture refined with product ranking layer
...
- Starting category: TBD after data exploration — likely All Beauty, Home & Kitchen, or Electronics based on review quality and volume
- Second category (for generalizability testing): added later if time allows
...
**Pipeline:**
1. Receives the selected product's structured insight object + the user's campaign brief
2. Gemini generates:
...
1. **Download Amazon Reviews 2023** — explore available categories, inspect review quality/volume/label distribution, pick the primary category.
